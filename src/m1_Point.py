...
class Point(object):
    ...
    def clone(self):
        point_clone = Point(self.x, self.y)
        return point_clone
    ...
    def move_to(self, x, y):
        ...
        if self.start_x > self.x:
            self.dx = (self.start_x - self.x)
        else:
            self.dx = (self.x - self.start_x)

        if self.start_y > self.y:
            self.dy = (self.start_y - self.y)
        else:
            self.dy = (self.y - self.start_y)
        ...
        self.distance = self.distance + math.sqrt(self.dx ** 2 + self.dy ** 2)
    ...
    def closer_to(self, point1, point2):
        dp1 = self.get_distance_from(point1)
        dp2 = self.get_distance_from(point2)
